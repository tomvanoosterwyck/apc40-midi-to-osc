...
import smallQues from './json/smallQues.json'

@Component
export default class SmallQue extends Vue {
  ...
  getColor (btn:any) {
    let rtn = ''
    if (btn.value) {
      rtn = this.colors[btn.class][btn.ledOn]
    } else {
      rtn = this.colors[btn.class][btn.ledOff]
    }
    return rtn
  }
  colors:any = {
    ...
    solo: [
      'display:inline;fill:#0000FF;opacity:0.3',
      'display:inline;fill:#0000FF;opacity:1'
    ],
    record: [
